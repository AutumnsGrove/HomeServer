...
## Project Purpose
Le Potato home server project: Transform a Libre Computer AML-S905X-CC (Le Potato) 2GB ARM SBC into a reliable, containerized 24/7 home server with remote access, network-wide ad blocking, NAS capabilities, development environment, and comprehensive monitoring.

## Tech Stack
- **Hardware**: Libre Computer AML-S905X-CC (Le Potato) 2GB, ARM Cortex-A53 64-bit
- **OS**: Ubuntu 24.04 LTS Server (ARM64)
- **Storage**: eMMC boot + External SSD via powered USB hub
- **Containerization**: Docker with overlay2 storage driver
- **Core Services**: Pi-hole, Tailscale VPN, Grafana, VictoriaLogs, Netdata
- **Development**: Claude Code, happy.engineering (on-demand)
- **Filesystem**: ext4 for external SSD storage
- **Networking**: Tailscale for remote access, Pi-hole for DNS/ad-blocking

## Architecture Notes
**Key Design Decisions:**
- **eMMC boot** (not microSD) for 4× faster performance and better reliability
- **VictoriaLogs** instead of Loki (87% less RAM usage - critical for 2GB constraint)
- **Powered USB hub** required for external SSD (prevents power-related instability)
- **Active cooling** mandatory for 24/7 operation (prevents thermal throttling)
- **ext4 over btrfs** for better Docker small-file performance
- **Development/monitoring mutually exclusive** to manage 2GB RAM constraint
- All services containerized with restart policies for self-healing
